...
$(document).ready(function(){      
	...
	function init_template(){ 
        ...
        function load_data_page()
        {
            ...
            // PAGE PRESENTATION
            if ( last_path_page == 'page-presentation.html')
            {
            ...
            // PAGE INSTALL
            if ( last_path_page == 'page-install.html')
            {
                // Remove loader
                $('#preloader').addClass('preloader-hide');
            }
           ...
           // PAGE WELCOME
            if ( last_path_page == 'welcome.html')
            {
            ...
            // PAGE USER
            if ( last_path_page == 'page-user.html')
            {
                //$('.app-page-calc').show();
                $('.app-page-info').hide();
                ...
                if ( user_mode == 'debit')
                {
                    $("#badge-card-message").html('vendre');
                    $("#header-title-mode").html('vendre');
                }
                else if ( user_mode == 'credit')
                {
                    $("#badge-card-message").html('recharge');
                    $("#header-title-mode").html('recharge');
                }	
                // Remove loader
                $('#preloader').addClass('preloader-hide');
            } 
            ...
            if ( last_path_page == 'page-transaction-list.html')
            {   
                if ( user_mode == 'debit')
                {
                ...
                // remove loader in transaction-list.js
            }
            
            

             // PAGE TERMIAL            
            if ( last_path_page == 'page-terminal.html')
            {   
                var terminal_number = window.localStorage.getItem("App-Terminal-Number");
                var terminal_name = window.localStorage.getItem("App-Terminal-Name");
                var currency = window.localStorage.getItem("App-Currency");
                
                var terminal_user = window.localStorage.getItem("App-Terminal-User");
                var terminal_username = window.localStorage.getItem("App-Terminal-UserName");
                var debit_commission = window.localStorage.getItem("App-Percentage-Debit-Commission");
                var credit_commission = window.localStorage.getItem("App-Percentage-Credit-Commission");
                

                
                $("#terminal_number").html(terminal_number);
                $("#terminal_name").html(terminal_name);
                $("#currency").html(currency);
                $("#terminal_user").html(terminal_user);
                $("#terminal_username").html(terminal_username);
                $("#debit_commission").html(debit_commission);
                $("#credit_commission").html(credit_commission);
           ...
           /**************** PAGE ADMIN ****************/ 
            ...
            if ( last_path_page == 'page-admin-dashboard.html')
            {
                 //DrawAllChart();
                ChartDasboard(DrawChart);
                 // Remove loader
                $('#preloader').addClass('preloader-hide');
            }
            
             // PAGE ADMIN INIT            
            if ( last_path_page == 'page-admin-init.html')
            { 
                $('.app-page-info').hide();
                $('.app-page-init').show();
            ...
            // PAGE ADMIN LOG LIST             
            if ( last_path_page == 'page-admin-log-list.html')
            {   
            ...
            // PAGE ADMIN TRANSACTION TERMINAL  
            if ( last_path_page == 'page-admin-terminal.html')
            {   
                var terminal_number = window.localStorage.getItem("App-Terminal-Number");
                var terminal_name = window.localStorage.getItem("App-Terminal-Name");
                ...
                $("#terminal-number").val(terminal_number);
                $("#terminal-name").val(terminal_name);
                $("#terminal-url").val(terminal_url);
                $("#terminal-currency").val(currency);
                $("#terminal-user").val(terminal_user);
                $("#terminal-username").val(terminal_username);
                $("#terminal-commission-debit").val(debit_commission  + ' %');
                $("#terminal-commission-credit").val(credit_commission + ' %');
            ...
            $('[data-toggle-theme], [data-toggle-theme-switch]').off().on('click',function(){
                ...
                $('body').toggleClass('theme-light theme-dark');
                    ...
                    if($('body').hasClass('theme-light')){
                        ...
                        localStorage.setItem('Appeca-Theme','light-mode')
                    }
                    if($('body').hasClass('theme-dark')){
                        darkSwitch.prop('checked', true); 
                        localStorage.setItem('Appeca-Theme','dark-mode')
                    }
                setTimeout(function(){allowTransitions();},150);
                return false;
            })
            if (localStorage.getItem('Appeca-Theme') == "dark-mode") {darkSwitch.prop('checked', true); $('body').removeClass('theme-light').addClass('theme-dark');}
            if (localStorage.getItem('Appeca-Theme') == "light-mode") {darkSwitch.prop('checked', false); $('body').removeClass('theme-dark').addClass('theme-light');}
        ...
        localStorage.setItem('z_Appeca-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    localStorage.getItem('z_Appeca-Visited:' + element.pathname) ||
                    localStorage.getItem('z_Appeca-Visited:' + element.pathname + '/')) {
                    if (className) {
        ...
        $('.list-group a').on('click',function(){if($(this).attr('href') != '#'){localStorage.setItem('z_Appeca-Visited-Scroll-Position', $(window).scrollTop());}})
        if($('.check-visited').length){
            setTimeout(function(){
                var getScroll = localStorage.getItem('z_Appeca-Visited-Scroll-Position');
                $(window).scrollTop(getScroll)
                 localStorage.removeItem('z_Appeca-Visited-Scroll-Position');
            },200);
        }
        ...
        if(visitDetection.length){
            var lastVisitValue = localStorage.getItem('Appeca-Last-Visited');
            var d = new Date();
            ...
            if(!lastVisitValue){
                $('.never-visited').show();
                $('.before-visited').hide();
                localStorage.setItem('Appeca-Last-Visited', '<br>Your last visit was ' + strDate +' at '+ strTime)
            } else {
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        setTimeout(function(){
            ...
            $('.single-slider-left').owlCarousel({loop:true, margin:20, nav:false, stagePadding:40, lazyLoad:true, items:1, autoplay: true, autoplayTimeout:4000});		
            $('.quad-slider').owlCarousel({loop:true, margin:20, nav:false, lazyLoad:true, items:4, autoplay: true, autoplayTimeout:4000});		
            $('.cover-slider').owlCarousel({loop:true, margin:0, nav:false, lazyLoad:true, items:1, autoplay: true, autoplayTimeout:6000});		
            $('.double-slider').owlCarousel({loop: true, stagePadding:10, margin: 20, nav: false, items: 2,  dots: false, autoplay: true, autoplayTimeout:3000});		
            ...
            $('[data-card-height]').each(function(){
                ...
                if(cardHeight == "cover"){
                    if(header.length && menuFooter.length){
                       ...
                       $('.map-full, .map-full iframe').css('height', windowHeight - header.height()) 
                    } else {
                       $(this).css('height', windowHeight)
                ...
                if(cardHeight == "cover-card"){
                   $(this).css('height', windowHeight+1) 
                   $('.map-full, .map-full iframe').css('height', windowHeight+1) 
                }
            });
        }
        card_extender();
        ...
        function highlightLoader(){
            $('[data-highlight-style]').on('click',function(){
                ...
                localStorage.setItem('Appeca-Highlight', getHighlight);
                $('#highlight').remove();
                $('head').append( $('<link rel="stylesheet" id="highlight" type="text/css" />').attr('href', getHighlight) );
                return false;
            });
            var rememberHighlight = localStorage.getItem('Appeca-Highlight');
            ...
            if (isMobile.Android()) {
                function showInstallPrompt(){
                    if($('#menu-install-pwa-android, .add-to-home').length){
                        if (localStorage.getItem('Appeca-PWA-Prompt') != "install-rejected") {
                            setTimeout(function(){
