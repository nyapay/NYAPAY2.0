...
var InforTerminal = function(successCallback){
    databaseHandler.db.transaction(function(transaction){
        transaction.executeSql(("select id, terminal_name, terminal_number, terminal_user, terminal_username,terminal_url, date, terminal_mode, terminal_name, terminal_currency, terminal_currency_symbol, terminal_version from terminal order by 1 desc limit 1"), [],
...
addTransactions: function(card_number, terminal_number, terminal_user, amount, commission, percentage, type, key, card_amount){
    databaseHandler.db.transaction(
        function(tx){
            var comment = '';
            tx.executeSql(
                "insert into transactions(card, card_amount, terminal_number, terminal_user, amount, commission, percentage, type_id, comment, key) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [card_number, card_amount, terminal_number, terminal_user, amount, commission, percentage, type, comment, key],
                function(tx, results){

                    AddLogSql('INFO', 'Transaction', 'Transactions Ajouté : [Carte:' + card_number + ']  [Montant Carte:' + card_amount + ']  [Montant:' + amount + ']  [Type:' + type + ']');
                },
                function(tx, error){ AddLogSql( errCallback(error.message) ) });
        },
        function(error){},
...
UpdateTerminalInformationSql: function(terminal_user, terminal_username, terminal_name, terminal_mode, credit_commission, debit_commission, terminal_currency, terminal_currency_symbol, terminal_version){
    databaseHandler.db.transaction(
        function(tx){
            tx.executeSql(
                "update terminal set terminal_name = ?, terminal_username = ?, terminal_mode = ?, credit_commission = ?, debit_commission = ?, terminal_currency = ?, terminal_currency_symbol = ?, terminal_version = ? where terminal_user= ?",
                [terminal_name, terminal_username, terminal_mode, credit_commission, debit_commission, terminal_currency, terminal_currency_symbol, terminal_version, terminal_user],
                function(tx, results){
					AddLogSql('INFO', 'Terminal', 'Mise à jour des informations du terminal');
                },
                function(tx, error){ AddLogSql( errCallback(error.message) ) });
        }
    );
},
UpdateTerminalSql: function(terminal_id, terminal_mode, terminal_url){
    databaseHandler.db.transaction(
        function(tx){
            tx.executeSql(
                ...
                function(tx, results){
                    toastr.warning("Le terminal a été mis à jour. Toutefois, vous pouvez perdre les modifications si le terminal est connecté au serveur", "Terminal");	
					AddLogSql('INFO', 'Terminal', 'Mise à jour des informations du terminal par Admin');
                },
                function(tx, error){ AddLogSql( errCallback(error.message) ) });
        }
    );
